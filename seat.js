...
class Seats {
    ...
    static getRookCannonMoveSeat_Lines(seat) {
        let seat_lines = [[], [], [], []];
        let row = Seats.getRow(seat); //this指类，而不是实例
    ...
    getSideNameColSeats(color, name, col) {
        return this.getSidenNameSeats(color, name).filter(s => Seats.getCol(s) == col);
    }
